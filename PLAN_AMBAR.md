...
- **Parámetros de la búsqueda:** `search_depth: 'basic'` (no `'advanced'`, que consume más cuota por búsqueda) y `max_results: 5`. Cada resultado se trunca a 500 caracteres de contenido antes de mandarlo a Gemini — son varios resultados por búsqueda y van al contexto de un modelo flash-lite, no hace falta el artículo entero para que conteste.
- **Autenticación contra Tavily:** header `Authorization: Bearer <key>`, no `api_key` en el body (es la forma vigente de la API de Tavily; poner la key en el body es el patrón viejo).
