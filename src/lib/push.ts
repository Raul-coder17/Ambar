...
export async function subscribeToPush(userId: string): Promise<PushStatus> {
  ...
  if (!sub) {
    sub = await reg.pushManager.subscribe({
      ...
      // TS 5.7+ distingue Uint8Array<ArrayBufferLike> de BufferSource (que
      // pide un ArrayBuffer concreto) — el array que devolvemos siempre está
      // respaldado por uno, el cast sólo asienta lo que ya es cierto en runtime.
...
// Da de baja la suscripción de este navegador: la borra localmente y de
// push_subscriptions.
